@extends('layouts.app')
@section('meta_title',$categoriadetalle[0]['titulo'])
@section('meta_description',$categoriadetalle[0]['descripcion'])
@section('meta_keyword',$categoriadetalle[0]['palabrasClaves'])
  ...
  <!-- Slides -->
    ...
    <div class="swiper-slide">
      <div class="flex flex-col shadow overflow-hidden">
        <div loading="lazy" class="flex-shrink-0 h-48 w-full object-cover" style="background: url('{{ asset($categoriadetalle[0]['imgBanner']) }}') center; background-size: cover;">
          ...
          <div class="grid grid-cols-1 fload md:grid-cols-2 gap-10 mx-4">
            <div class="ml-20 items-center mt-20">
              <h2 class="text-2xl rounded-md text-white font-bold uppercase">CURSO DE {{ __($categoriadetalle[0]['categoria']) }}</h2>
            </div>
      
              @php
                $fechaActual = date('Y-m-d H:i:s');
                if ($categoriadetalle[0]['finOferta'] > $fechaActual) {
                  echo'<div class="text-white mt-4 text-right mr-20"> <h2 class="text-3xl font-bold">'.__("OFERTAS ESPECIALES").'</h2>';
                  if ($categoriadetalle[0]['descuento'] == 0 || $categoriadetalle[0]['oferta'] != 0) {
                    echo'<h2 class="text-2xl font-bold">'.__("TODOS LOS CURSOS A").'$'.$categoriadetalle[0]['oferta'].'</h2>';
                  }if ($categoriadetalle[0]['descuento'] != 0 || $categoriadetalle[0]['oferta'] == 0) {
                    echo'<h2 class="text-2xl font-bold">'.__("CURSOS CON").' '.$categoriadetalle[0]['descuento'].'% '.__("DE DESCUENTO").'</h2>';
                  }
                  $tiempo1 = new DateTime($categoriadetalle[0]['finOferta']);
                  ...
                  echo'<div id="clockdiv" date="'.$categoriadetalle[0]['finOferta'].'"
...
<div class="w-full mt-8 mb-6 mx-auto px-4 flex items-center justify-between">
  <span class="text-2xl">
    ...
    <strong class="text-2xl">{{ __($categoriadetalle[0]['categoria']) }}</strong>
  </span>
  <div class="flex items-center select-none invisible sm:invisible md:visible lg:visible xl:visible">
      <a href="" class="ml-2 flex items-center border
        py-2 px-3 leading-none text-white bg-blue850 hover:bg-blue750 rounded-md p-2 focus:outline-none">
          <svg class="h-3 w-3 fill-current mr-1" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg"
              xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 349.03 349.031"
...
<div class="w-screen sm:w-full md:full lg:w-full xl:w-full mx-auto grid grid-cols-1 px-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 mb-10">
  @foreach ($cursoscategory->results as $key => $item)
  <div class="">
    <a href="{{ 'https://click.linksynergy.com/deeplink?id=sIt9MIeGnaM&mid=47900&murl=https%3A%2F%2Fwww.udemy.com'.$item->url }}" rel="nofollow" target="_blank" class="c-card block bg-white shadow-md hover:shadow-xl rounded-lg overflow-hidden">
          <!--<a href="{{config('services.cred_afiliado.url').''.config('services.cred_afiliado.id').'&offerid='.config('services.cred_afiliado.offerid').'.'.$item->id.'&type=2&murl=https%3A%2F%2Fwww.udemy.com'.$item->url}}" rel="nofollow" target="_blank" class="c-card block bg-white shadow-md hover:shadow-xl rounded-lg overflow-hidden">-->
      ...
      <div class="p-4 border-t border-b">
        <div class="grid grid-cols-2">
            <div class="col-span-2 text-center">
              <button href="{{ 'https://click.linksynergy.com/deeplink?id=sIt9MIeGnaM&mid=47900&murl=https%3A%2F%2Fwww.udemy.com'.$item->url }}" rel="nofollow" target="_blank" class="text-sm bg-blue850 hover:bg-blue750 text-white font-bold py-2 px-6 rounded-full inline-flex items-center">
...
@php
$totalpaginas= ceil($cursoscategory->count/20);
@endphp
<div class="container mx-auto items-center content-center text-center">
    ...
    <nav id="uk-pagination" class="relative z-0 content-center inline shadow-sm" lan="{{app()->getLocale()}}" url="{{ url('/courses') }}"
            pagina="{{ $page }}" tipo="category" totalpage="{{ $totalpaginas }}"
            cat="{{ $categoriadetalle[0]['ruta'] }}">
        </nav>
    </div>
</div>
